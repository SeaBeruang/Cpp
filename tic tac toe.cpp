...
class TicTacToe
{
   ...
   void PlayerMove(char num, char Player)
   {
      ...
      bool CheckHalfRound(char Player, bool HalfRound)
   {
   		for(int i = 0; i < 3; i++) //Increment i to check rows
      {
         if(table[i][0] == table[i][1] && table[i][1] == table[i][2]) HalfRound = true;
      }
      for(int i = 0; i < 3; i++) //Increment i to check column
      {
         if(table[0][i] == table[1][i] && table[1][i] == table[2][i]) HalfRound = true;
      }
      ...
      if(HalfRound == true)
      {
         cout << "Player " << Player << " wins the Half Round!\n\n";
      }
         return HalfRound;
   } //End Check Winner
...
int main()
{
    bool done = false, GameOver = false, HalfRound = false;
    char Player = 'O', num;
    TicTacToe myGame;
    myGame.SetTable();
    do
    {
        ...
        HalfRound = myGame.CheckHalfRound(Player, HalfRound);
